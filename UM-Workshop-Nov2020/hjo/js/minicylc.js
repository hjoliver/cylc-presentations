...
/* HJO: modified to show reflow - only works until the back flow catches up to
 * succeeded tasks of the front flow (because: dependency matching not SoD). */
...
var theme = {
    ...
    'running_stroke': 'black',
	  'running_text': 'white',
    'succeed_fill': '#c3eeef',
    'succeed_stroke': 'slategray',
	  'succeed_text': 'slategray'
}
...
class MiniCylc {
    ...
    constructor(div, show_points) {
        ...
        this.theme = theme;
				if ($(div).data('succeed_fill') !== undefined) {
					 this.theme['succeed_fill'] = $(div).data('succeed_fill')
			  }
        ...
        if ($(div).data('focus') === true) {
            ...
            this.focus = false;
				...
				// show reflow
        if ($(div).data('reflow') ) {
            this.reflow_task = $(div).data('reflow');
        } else {
            this.reflow_task = undefined;
        }
    ...
    _style_node(node, fill, stroke, text) {
        /**
        ...
        if (!text) {
            text = 'black';  // Default to black text.
        }
    ...
    _init() {
        ...
        for (let task in this.nodes) {
            this.waiting.add(task);
				...
				if ( this.reflow_task ) {
					 this.waiting.delete(this.reflow_task);
					 this.running.add(this.reflow_task);
		    }	
        this._style();
    }
